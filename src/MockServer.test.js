...
// このテストファイルの最初に１回だけ実行される。
beforeAll(() => server.listen());
afterEach(() => {
  server.resetHandlers(); // Udemy講師によると「呼ぶ決まりになってる」らしい。。。
  cleanup();
});
// このテストファイルの最後に１回だけ実行される。
...
describe("Mocking API", () => {
  ...
  it("[Fetch failure]Should display error msg, no render heading and button abled", async () => {
    // server.use()で、setupUserで定義したものに対して挙動を上書きできる。このテストケースだけ挙動が変わる。
    server.use(
      rest.get(
